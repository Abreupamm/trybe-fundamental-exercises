...
const pontos = (respostaCorreta, respostaEstudante) => {
  if (respostaCorreta === respostaEstudante) {
    return 1;
  }
...
const correcaoGabarito = (gabarito, resposta, callback) => {
  let pontuacao = 0;
  for (let i = 0; i < gabarito.length; i += 1) {
    const correcao = callback(gabarito[i], resposta[i]);
    pontuacao += correcao;
  }
  return `Nota final: ${pontuacao} pontos`;
};

console.log(correcaoGabarito(RIGHT_ANSWERS, STUDENT_ANSWERS, pontos));
